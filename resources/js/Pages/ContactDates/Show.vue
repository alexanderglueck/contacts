...
<template>
    <AppLayout :title="`${item.name} — ${contact.fullname}`">
        ...
        <div class="text-sm">
            <Link :href="route('contact_dates.index', contact.ulid)" class="text-indigo-600 hover:text-indigo-500">
                &larr; Back to dates
            </Link>
        </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <div class="flex gap-2 text-sm">
                    <Link
                        v-if="can.edit"
                        :href="route('contact_dates.edit', [contact.ulid, item.ulid])"
                    ...
                    <Link
                        v-if="can.delete"
                        :href="route('contact_dates.delete', [contact.ulid, item.ulid])"
            ...
            <dl class="px-6 py-4 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                ...
                <div>
                    ...
                    <dd class="text-gray-900">{{ item.formatted_date }}</dd>
                </div>
                <div>
                    <dt class="font-medium text-gray-700">Skip year</dt>
                    <dd class="text-gray-900">{{ item.skip_year ? 'Yes' : 'No' }}</dd>
                </div>
            </dl>
        </div>
    </AppLayout>
