    ...
    <AppLayout :title="`${contact.fullname} — Phone numbers`">
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                ...
                <Link v-if="canCreate" :href="route('contact_numbers.create', contact.ulid)">
                    <PrimaryButton type="button">Add phone number</PrimaryButton>
                </Link>
            </div>
            ...
            <ul v-else class="divide-y divide-gray-200">
                <li v-for="item in items" :key="item.id">
                    <Link
                        :href="route('contact_numbers.show', [contact.ulid, item.ulid])"
