...
<template>
    <AppLayout :title="`${item.name} — ${contact.fullname}`">
        ...
        <div class="text-sm">
            <Link :href="route('gift_ideas.index', contact.slug)" class="text-indigo-600 hover:text-indigo-500">
                &larr; Back to gift ideas
            </Link>
        </div>
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <div class="flex gap-2 text-sm">
                    <Link
                        v-if="can.edit"
                        :href="route('gift_ideas.edit', [contact.slug, item.ulid])"
                    ...
                    <Link
                        v-if="can.delete"
                        :href="route('gift_ideas.delete', [contact.slug, item.ulid])"
            ...
            <dl class="px-6 py-4 space-y-3 text-sm">
                ...
                <div v-if="item.description">
                    ...
                    <dd class="text-gray-900 whitespace-pre-line">{{ item.description }}</dd>
                </div>
                <div v-if="item.url">
                ...
                <div v-if="item.formatted_due_at">
                    <dt class="font-medium text-gray-700">Due at</dt>
                    <dd class="text-gray-900">{{ item.formatted_due_at }}</dd>
                </div>
            </dl>
        </div>
    </AppLayout>
