...
const submit = () => form.delete(route('gift_ideas.destroy', [props.contact.ulid, props.item.ulid]));
</script>
    ...
    <AppLayout :title="`${contact.fullname} — Delete ${item.name}`">
        ...
        <form @submit.prevent="submit" class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Delete gift idea {{ item.name }}?</h2>
            </div>

            <div class="px-6 py-4 text-sm text-gray-700">
                <p>This will permanently remove this gift idea from {{ contact.fullname }}.</p>
            </div>
