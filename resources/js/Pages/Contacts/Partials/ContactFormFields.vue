...
<template>
    <div class="space-y-8">
        <!--
            Identity — what you'd ask someone for first. Name on top so
            the form starts where the user's attention naturally goes;
            the title/salutation row below keeps honorifics accessible
            without forcing them in front of the name. Short fields share
            a 12-column row on sm+ so a "Dr." input doesn't eat a full
            line. Mobile (default) stacks everything.
        -->
        <fieldset class="space-y-4">
            <legend class="text-sm font-semibold text-gray-900 mb-2">{{ t('contacts.form_sections.identity') }}</legend>
                ...
                <div>
                    ...
                    <Select id="gender_id" v-model="form.gender_id" required>
                        <option v-for="gender in genders" :key="gender.id" :value="gender.id">
                            {{ te(`contacts.gender.${gender.gender}`) ? t(`contacts.gender.${gender.gender}`) : gender.gender }}
                        </option>
                    </Select>
        ...
        <!--
            Personal — life-events, identifying data not tied to work,
            and free-form context. DOB pairs with IBAN (the IBAN gets
            recorded for friends too: paying back a coffee, splitting
            rent, etc.), with death info on its own row. first_met and
            note are the only widgets that genuinely benefit from full
            width.
        -->
        <fieldset class="space-y-4">
            <legend class="text-sm font-semibold text-gray-900 mb-2">{{ t('contacts.form_sections.personal') }}</legend>
        ...
        <!--
            Work — company gets the wide row (long names are common),
            job/department share a row, and VATIN + custom_id (the two
            remaining short ID-style fields) sit on one row.
        -->
        <fieldset class="space-y-4">
        ...
        <!--
            Categorisation — how the contact slots into the user's own
            taxonomy. Groups + active/inactive, nothing else.
        -->
        <fieldset class="space-y-4">
            <legend class="text-sm font-semibold text-gray-900 mb-2">{{ t('contacts.form_sections.categorisation') }}</legend>
